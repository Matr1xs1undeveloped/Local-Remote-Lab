...
open the terminal by pressing Ctrl + Alt + T or searching for it in ur applications
after that we need to go into the lab folder by executing this command in terminal: "cd ~/Desktop/Lab" if your lab folder is somewhere else feel free to change the path
and while were at it make sure python is installed using this command: "python3 --version" or "python --version" if it shows a version ur set with python
if not, install python:
ubuntu/Debian: sudo apt update, sudo apt install python3 python3-pip
Fedora: sudo dnf install python3 python3-pip

after that, make sure to install the required modules: "pip3 install flask pillow psutil waitress" (project doesn't have a requirements.txt because there can be some expected bugs with it or errors)
after you have done all of this, try to run the server: "python3 server.py" or "python server.py" after u ran it, it should say the server is listening on all interfaces or just naming a port (5000 if u didn't change it)

now inorder to visit the website you need to find ur local ip address using ip addr or hostname -I, look for something looking like: 192.168.x.x

last step: open the website and enjoy your remote access control
on your phone or another device (if you want to) on the same network (important, do not use a vpn on any device when doing this neither the one where the server is running)
then open a browser and type in: http://youripaddress:5000 (replace youripaddress with ur local one that u got from the commands)
have fun!

NOTE: if you encounter any problems add me on discord: morggie30, describe ur problem clearly and send screenshots
